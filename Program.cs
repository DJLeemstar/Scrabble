...
namespace ScrabbleProject
{
    class Program
    {
        void pcMyMain()     // Main Procedure. 
        {
            ...
            // For example, 'rgch' notation indicates "Range + Character", indicating Character Array.
        ...
        bool fnWordTrue(string strInput, char[] rgchLetters)        // Function to return whether the user's input can be made with the given characters.
        {
            ...
            char[] rgchDupeLetters = new char[7];

            for (int i = 0; i < 7; i++)                              // Copies values from random Letters array into a duplicate array.
            {

                rgchDupeLetters[i] = rgchLetters[i];                // Keeps a copy to revert to after the next For loop that removes avlues.
